-- Apply with the Supabase SQL editor if migrations are not deployed by the CLI.
...
alter table public.failed_payments drop constraint if exists failed_payments_audience_segment_check;
alter table public.failed_payments add constraint failed_payments_audience_segment_check
  check (audience_segment in ('subscription', 'standalone', 'unknown')) not valid;
alter table public.failed_payments validate constraint failed_payments_audience_segment_check;

alter table public.recovery_sequences
  add column if not exists audience_segment text not null default 'unknown';
alter table public.recovery_sequences drop constraint if exists recovery_sequences_audience_segment_check;
alter table public.recovery_sequences add constraint recovery_sequences_audience_segment_check
  check (audience_segment in ('subscription', 'standalone', 'unknown')) not valid;
alter table public.recovery_sequences validate constraint recovery_sequences_audience_segment_check;
...
update public.failed_payments set audience_segment = case
