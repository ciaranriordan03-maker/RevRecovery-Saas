...
export function RecoverySettingsForm({
  ...
  const emailChanged =
    JSON.stringify({ email: settings.email, tone: settings.recovery.defaultEmailTone }) !==
    JSON.stringify({
      email: savedSettings.email,
      tone: savedSettings.recovery.defaultEmailTone,
    });
  ...
  async function saveEmailIdentity(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setSavingEmail(true);
    setEmailStatus("");
    ...
    try {
      ...
      if (!response.ok || !payload.settings) {
        throw new Error(getErrorMessage(payload, "Unable to save email identity."));
      }

      const nextSettings = cloneSettings(payload.settings);
      setSettings(nextSettings);
      setSavedSettings(cloneSettings(nextSettings));
      setEmailStatus("Email identity saved.");
      router.refresh();
    } catch (error) {
      setEmailStatus(
  ...
  return (
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <form
        className="rounded-[var(--radius-card)] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow-card)]"
        onSubmit={saveEmailIdentity}
      >
        <h2 className="text-base font-medium text-[var(--foreground)]">Email identity</h2>
        <p className="mt-2 text-sm leading-5 text-[var(--muted-strong)]">
          Set the sender customers see and the addresses used for replies and support.
        </p>
        ...
        <label className="mt-5 block text-sm font-medium text-[var(--foreground)]">
          ...
          <select
            ...
            onChange={(event) =>
              setSettings((current) => ({
                ...current,
                recovery: { ...current.recovery, defaultEmailTone: event.target.value },
              }))
            }
        ...
        <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Button disabled={!emailChanged || savingEmail} type="submit">
            {savingEmail ? "Saving…" : "Save email identity"}
          </Button>
        </div>
      </form>
    </div>
