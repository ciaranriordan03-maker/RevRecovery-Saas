...
export function AuthProgressLabel({ isCheckEmail }: { isCheckEmail: boolean }) {
  return (
    <div className="mb-5 flex items-center gap-2 text-xs font-medium text-[var(--muted)]">
      ...
      >
        {isCheckEmail ? "Step 2 of 2" : "Step 1 of 2"}
      </span>
      <span>{isCheckEmail ? "Verify your email" : "Create or access account"}</span>
    </div>
  );
}
